...
//可以参考$MLT_DATA/core/loader.dict
 ...
 *
 * 如果name是文件名，则从文件初始化
 * 如果不是，则从PRESET_PATH/{type}/{service}/[{profile}/]{name}.初始化properties。
 * 其中profile可能为空。
 * type,service,provile均是子类对象对应的mtl_type,mlt_service/MLT_PROFILE属性
 *
 * \public \memberof mlt_properties_s
 * \param self a properties list
 ...
 * 获得that对象所有可序列化的有prefix前缀的属性值。拷贝时，属性为去掉prefix的部分
 *
 ...
 *
 * properties只需要保护hash结构本身。返回的mlt_property本身自带锁
 */
...
mlt_position mlt_properties_get_position( mlt_properties self, const char *name )
{
	...
	if ( value )
	{
		...
		double fps = mlt_profile_fps( profile );
		property_list *list = self->local;
		//如果字段值存储的是表示时戳的字符串，则需要fps将其转换为帧编号
