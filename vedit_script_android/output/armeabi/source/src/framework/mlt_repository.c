...
struct mlt_repository_s
{
	...
	/**
	 * 下列各成员的结构
	 * 		service_name->properties组成的hash
	 * 		其中properties value的
	 * 			symbol--->service   实例的工厂函数
	 * 			metadata--->properties	service  repository 全局的metadata，不是service某个实例的metadata
	 * 			metadata_cb-->callback  service全局的metadata的获取回调
	 * 			metadata_cb_data->callback arg  metadata的回调参数
	 *
	 */
...
mlt_repository mlt_repository_init2(const char** plugins, size_t count)
{
	if ( !plugins || count <= 0 )
		return NULL;
	...
	int i, plugin_count = 0;
	for ( i=0; i<count; i++) {
		/*
		 * RTLD_LAZY  当被加载的动态库的某个函数被调用时，它调用了未解析符号时，才从其他动态库绑定缺少的符号。仅局限于函数符号。
		 * RTLD_NOW 当被加载动态库时，如果有为解析符号，则立即解析。
		 * RTLD_GLOBAL 被加载的动态库的符号可以被后续加载的其他动态库绑定
		 * RTLD_LOCAL RTLD_GLOBAL的反面。不指定时，等价于RTLD_LOCAL
		 *
		 */
		if (plugins[i] == NULL || strncmp(plugins[i], "libmlt", strlen("libmlt")))
			continue;

		int flags = RTLD_NOW;

		if ( strstr( plugins[i], "libmltkino" ) || strstr(plugins[i],"mltkino")
				|| strstr(plugins[i],"kino") ) //这个库或者这个系列的库的符号被其他模块所使用
			flags |= RTLD_GLOBAL;
		...
		void *object = dlopen( plugins[i], flags );
		...
		if ( object != NULL )
		{
			...
			// 如果mlt_register存在，则调用它
			if ( symbol_ptr != NULL )
			{
				symbol_ptr( self ); //以repository为参数，调用模块的入口函数。

				// Register the object file for closure
				// 当repository析构时，dlclose对应so handle
				mlt_properties_set_data( &self->parent, plugins[i], object, 0, ( mlt_destructor )dlclose, NULL );
				++plugin_count;
			}
			else
			{
		...
		{
			mlt_log_warning( NULL, "%s: failed to dlopen %s\n  (%s)\n", __FUNCTION__, plugins[i], dlerror() );
		}
	}

	if ( !plugin_count )
		mlt_log_error( NULL, "%s: no plugins found for repository init ext\n", __FUNCTION__) ;
...
mlt_repository mlt_repository_init( const char *directory )
{
	...
	for ( i = 0; i < count; i++ )
	{
		...
		if ( strstr( object_name, "libmltkino" ) ) //这个库或者这个系列的库的符号被其他模块所使用
			flags |= RTLD_GLOBAL;
		//李磊尝试
		//flags |= RTLD_GLOBAL;
 ...
 *
 * 该函数主要被各模块调用，在模块的mlt_register中注册模块所支持的各service以及service的创建函数
 */
 ...
 *
 * 所有service的工厂方法的参数： profile,type,service名字，初始化参数
 */
 ...
 *
 * 主要用于各模块为其提供的service注册 metadata_cb回调，以及metadata_cb回调的参数
 */
 ...
 *
 * 获得service的repository 全局metadata数据。如果有metadata则返回，否则通过metadata_cb调用返回，同时metadata_cb返回的properties
 * 会缓存在repository中
 */
 ...
 * 递归遍历 MLT_PRESETS_PATH目录下的所有文件，形成以文件路径为key指向的预设properties集合
 */
