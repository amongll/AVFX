...
void FilterLoader::declare()
{
	MltLoader::regist_loader<FilterLoader>("filter", &FilterLoader::get_filter);
}

mlt_service FilterLoader::get_filter(JsonWrap js) throw (Exception)
{
	json_t* defines = js.h;
	...
	je = json_object_get(defines, "filter");
	...
	if ( !obj ) {
		throw_error_v(ErrorRuntimeLoadFailed, "filter load failed:%s", filter_id);
	}

	mlt_properties props = mlt_filter_properties(obj);
	...
	if (je && json_is_object(je) && json_object_size(je)) {
		...
		mlt_filter_set_in_and_out(obj,json_integer_value(inj), json_integer_value(outj));
